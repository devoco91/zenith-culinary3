...
const TransactionPage = () => {
  ...
  const renderStudentInformation = () => (
    <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {[['First Name', 'firstName'], ['Last Name', 'lastName'], ['Email Address', 'email'], ['Address', 'address', 2], ['City', 'city'], ['State', 'state']].map(
          ([label, name, span], idx) => (
  ...
  const renderPaymentInformation = () => (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg p-6 mb-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {[['Cardholder Name', 'cardName'], ['Expiry Date', 'expiryDate', 'MM/YY'], ['CVV', 'cvv', '•••']].map(
          ([label, name, placeholder], idx) => (
            <div key={idx}>
              <label className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
              <input
                type="text"
